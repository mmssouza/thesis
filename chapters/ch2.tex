...
Nesta matriz, a rede \ac{SOM} mapeou as três estruturas da classe vermelha ($A_1$, $A_2$ e $A_3$) com pouca separação inter-classes, uma vez que não há regiões escuras no entorno das mesmas. A estrutura $A_4$, que também pertence a classe vermelha, encontra-se mais afastada das estruturas referidas anteriormente, pois há uma barreira mais escura a separando das demais. Há também uma grande barreira que separa as classes entre si. As quatro estruturas da classe vermelha, por exemplo, estão bem separadas das demais, pertencentes a classe azul ($B_1$, $B_2$, $B_3$ e $B_4$) e verde ($C_1$).
